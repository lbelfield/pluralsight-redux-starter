...
describe('CourseList via Enzyme', () => {
    it('test the nav text values', () => {
        ...
        // <Link /> has text: Authors and link to: course
        expect(wrapper.find('nav').childAt(3).props().to).toEqual('authors');
        expect(wrapper.find('nav').childAt(3).props().children).toEqual('Authors');

        // <Link /> has text: Courses and link to: course
        expect(wrapper.find('nav').childAt(4).props().to).toEqual('courses');
        expect(wrapper.find('nav').childAt(4).props().children).toEqual('Courses');
    });
    ...
    it('test the loading dots, when loading set to true, returns dots', () => {
        ...
        expect(wrapper.find('nav').childAt(5).find('LoadingDots').props().inteval).toEqual(100);
        expect(wrapper.find('nav').childAt(5).find('LoadingDots').props().dots).toEqual(20);
    });
});